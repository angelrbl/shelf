...
def settings_page():
    ...
    with ui.column().classes('max-w-7xl w-full mx-auto px-4 md:px-0 py-4 gap-6 items-center pb-28 sm:pb-4'):
        with ui.row().classes('w-full justify-between items-end'):
            ui.label(_("settings_title")).classes("text-3xl text-slate-700 dark:text-neutral-200 font-medium")
            with ui.row().classes('items-center gap-1 cursor-pointer group').on('click', lambda: ui.navigate.to('/profile')):
                ...
                ui.label(_("settings_go_back")).classes('text-slate-700 dark:text-neutral-200 font-bold group-hover:text-slate-500 dark:group-hover:text-neutral-400')

        ui.separator().classes('w-full')

        section_title(icon='account_circle', text=_("settings_account"))
        render_account_settings(user=user)
        
        section_title(icon='key', text=_("settings_privacy"))
